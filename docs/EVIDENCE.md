...
add the gateway but a customer could never finish paying. With those fixed, the admin setup and
checkout path work again. Here is the admin check on the fixed module: activate, save the settings,
re-open the config page, and the settings are still there on both versions:
...
WHMCS's encrypted gateway settings without truncation. The admin form itself was not the PHP 8
fatal; the broken path was checkout and payment handling after the gateway was enabled.
...
- The small loading indicator near the top is the payment poll. In the screenshot run,
  `monero-wallet-rpc` was reachable and returned the integrated address; the wallet had no matching
  payment yet, so the poll stayed in the existing "Waiting for your payment" state. The detection
  logic in `verify.php` was not redesigned, only the PHP 8 compatibility shims were added.
