...
Tested in Docker on WHMCS 8.13.4 and 9.0.5 (PHP 8.3), with `monero-wallet-rpc` running against a
public stagenet daemon.
...
After the fix the pay page renders, the price is fetched (CoinGecko, with a Kraken fallback), and
the amount is converted to XMR. Verified on both WHMCS 8.13.4 and 9.0.5.
...
WHMCS's encrypted gateway settings without truncation. Together with the checkout below, the gateway
can be added, configured, and used end to end on PHP 8.x.

## Checkout, local QR branch (`fix-php8-local-qr`)

The checkout shows the amount, a per-payment integrated address, and a QR generated on the server
with the bundled MIT library. No request leaves the server for the QR.
...
- The small loading indicator near the top is the payment poll. The test wallet-rpc ran in offline
  mode, so the poll could not reach a daemon; against a live daemon it reads "Waiting for your
  payment" and the invoice is marked paid once the payment confirms. The detection logic in
  `verify.php` was not changed, only the PHP 8 compatibility shims (the deprecated filter and the
  null guard).
